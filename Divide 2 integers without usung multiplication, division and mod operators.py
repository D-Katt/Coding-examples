...
# В зависимости от величины делимого и делителя результат может получаться
# с разной степенью округления или без округления.
...
sign = -1  # Переменная для определения знака результирующего числа

# Если оба числа положительные, переменная sign не изменится.
# Если оба числа отрицательные, переменная sign не изменится,
# для чисел меняем знак на противоположный.
# Если только одно из двух чисел отрицательное, переменная sign изменит значение.
if dividend < 0:
    sign = -sign
    dividend = -dividend
...
# Последовательно вычитаем делитель из делимого, пока делимое не пересечет 0.
# Подчитываем количество операций вычитания.
# Выводим переменную count, уменьшив ее на 1.
while dividend >= 0:
    dividend -= divisor
    count += 1
count -= 1

if sign == 1:  # Если переменная sign имеет положительное значение,
    # меняем знак полученного числа.
    count = -count
