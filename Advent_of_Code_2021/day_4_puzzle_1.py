...
As soon as there is a complete row or column on any board,
calculate the score of the winning board. Start by finding the sum
of all unmarked numbers on that board. Then, multiply that sum by the number
that was just called when the board won.
"""
...
for num in numbers:
    ...
    for board in np.split(all_boards, n_boards):
